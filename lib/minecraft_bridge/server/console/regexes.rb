  ...
  module Server
    module Console
      module Regexes
        ...
        SERVER_CANT_KEEP_UP = /^Can't keep up! Did the system time change, or is the server overloaded\? Running (\d+)ms behind, skipping (\d+) tick\(s\)$/
        SERVER_TIME_BACKWARDS = /^Time ran backwards! Did the system time change\?$/
        SERVER_CONSIDERING_CRASHED = /^Considering it to be crashed, server will forcibly shutdown.$/
        SERVER_TICK_TIME = /^A single server tick took (\d+,?\d*) seconds \(should be max (\d+,?\d*)\)$/

        SERVER_FAILED_PORT = /^\*\*\*\* FAILED TO BIND TO PORT!$/
        SERVER_EXCEPTION = /^The exception was: (.+)$/
        SERVER_PERHAPS_ALREADY_RUNNING = /^Perhaps a server is already running on that port\?$/

        SERVER_CRASH_REPORT = /^This crash report has been saved to: (.+)$/
